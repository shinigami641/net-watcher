...
@api.route("/info-detail-client/<ip>")
...
@api.route("/traffict-stop", methods=["POST"])
def traffic_stop():
    payload = request.get_json(silent=True)
    if payload is None:
        return error_response("Invalid or missing JSON payload", http_status=400, app_code=APP_ERROR_CODES["INVALID_INPUT"])
...
@api.route("/traffict-status", methods=["GET"])
def traffic_status():
...
@api.route("/traffict-stop-all", methods=["POST"])  # Ubah ke POST
...
@api.route("/traffict-emergency-stop", methods=["POST"])
def emergency_stop():
